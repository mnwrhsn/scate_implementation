...
void PCA9685::setAllPWM(int on_value, int off_value) {
	...
	if (fd != -1) {
		...
		write_byte(fd, ALLLED_ON_L, on_value & 0xFF);
		
		write_byte(fd, ALLLED_ON_H, on_value >> 8);
				
		write_byte(fd, ALLLED_OFF_L, off_value & 0xFF);
		
		write_byte(fd, ALLLED_OFF_H, off_value >> 8);
